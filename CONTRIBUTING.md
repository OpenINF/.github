...
Those texts arrive with placeholders in them. MIT opens on
`Copyright (c) <year> <copyright holders>`, and a copy that still says that
names nobody and grants nothing, so fill in the year and the holder before you
commit the file.

What that rules out is a `LICENSE.md` explaining an arrangement of licenses.
Prose about licensing is not a license, and somebody looking for the terms
should not have to sort one from the other first.
