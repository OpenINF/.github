...
own where a section is long enough to need them, as [ADR 0003][] does under
`Decision` and `Results`. `Results` is filled in after the decision has had time
to prove itself, which is usually well after the rest.
